    ...
    this.actions$.pipe(
      ...
      mergeMap((action: StreamLongTermGoal) => {
        ...
        return connection.loadStream.pipe(
          ...
          catchError((error) =>
            of(new StreamLongTermGoalFail(error, action.correlationId))
          )
        );
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap((action: LoadLongTermGoal) => {
        ...
        return connection.loadStream.pipe(
          ...
          catchError((error) =>
            of(new LoadLongTermGoalFail(error, action.correlationId))
          )
        );
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap((action: AddLongTermGoal) => {
        return this.db.addEntity('longTermGoals', action.longTermGoal).pipe(
          ...
          catchError((error) =>
            of(new AddLongTermGoalFail(error, action.correlationId))
          )
        );
      })
    )
  ...
  update$: Observable<Action> = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action: UpdateLongTermGoal) => {
        return this.db
          .updateEntity('longTermGoals', action.__id, action.changes)
          .pipe(
            mergeMap(() =>
              merge(
                of(
                  new UpdateLongTermGoalSuccess(
                    action.__id,
                    action.changes,
                    action.correlationId
                  )
            ...
            catchError((error) =>
              of(new UpdateLongTermGoalFail(error, action.correlationId))
            )
          );
      })
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap((action: UpsertLongTermGoal) => {
        return this.db.upsertEntity('longTermGoals', action.longTermGoal).pipe(
          mergeMap((results) =>
            merge(
              ...
              results.type === 'add'
                ...
                : this.actionsOnUpdate(
                    new UpdateLongTermGoal(
                      action.longTermGoal.__id,
                      action.longTermGoal,
                      action.correlationId
                    )
                  )
          ...
          catchError((error) =>
            of(new UpsertLongTermGoalFail(error, action.correlationId))
          )
        );
      })
    )
      ...
      mergeMap((action: RemoveLongTermGoal) => {
        return this.db.removeEntity('longTermGoals', action.__id).pipe(
          mergeMap(() =>
            merge(
              of(
                new RemoveLongTermGoalSuccess(action.__id, action.correlationId)
              ),
              this.actionsOnRemove(action)
            )
          ),
          catchError((error) =>
            of(new RemoveLongTermGoalFail(error, action.correlationId))
          )
        );
      })
    )
